...
namespace LCU {
	inline namespace Algorithms {
		...
		char* Encryption::CaesarCipher(const char* const p_stringToCipher, int shift) {
			...
			for (size_t i = 0; i < stringToCipherLength; ++i) {
				if (Validation::IsAlpha(result[i])) {
					if (Validation::IsLowerCase(result[i]) && Conversion::ToASCII(result[i]) + shift > 122
						|| Validation::IsUpperCase(result[i]) && Conversion::ToASCII(result[i]) + shift > 90) {
						result[i] -= 26;
					}
					result[i] += shift;
				}
		...
		char* Encryption::VigenereCipher(const char* const p_stringToCipher, char* const p_key) {
			...
			for (c = 0; c < stringToCipherLength; ++c) {
				if (Validation::IsAlpha(result[c])) {
					result[c] = vigenereTable[Conversion::ToASCII(Conversion::ToUpperCase(p_stringToCipher[c])) - 64][Conversion::ToASCII(Conversion::ToUpperCase(result[c])) - 64];
				}
			}
			...
			for (c = 0; c < stringToDecipherLength; ++c) {
				if (Validation::IsAlpha(result[c])) {
					for (size_t j = 1; j < 27; ++j) {
						if (vigenereTable[Conversion::ToASCII(Conversion::ToUpperCase(result[c])) - 64][j] == Conversion::ToUpperCase(p_stringToDecipher[c])) {
