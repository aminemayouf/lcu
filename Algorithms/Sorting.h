...
namespace LCU {
	inline namespace Algorithms {
		...
		public:
			...
			static void CountingSort(int*const p_pArray, int* p_pArray_out, int p_size, int p_maxValue);
		};
		...
		void Sorting<Type>::SelectionSort(Type* p_pArray, size_t p_size) {
			...
			while (p_size > 0) {
				...
				for (size_t i = 1; i < p_size; ++i) {
					if (p_pArray[i] > p_pArray[max]) { // finds the maximum element
						max = i;
					}
		...
		void Sorting<Type>::SelectionSort(Type* p_pArray, size_t p_startIndex, size_t p_endIndex) {
			...
			while (p_endIndex + 1 > 0) {
				...
				for (size_t i = p_startIndex + 1; i < p_endIndex + 1; ++i) {
					...
					Utility::Swap(p_pArray[max], p_pArray[p_endIndex]); // putting maximum element on its proper position.
				}
				--p_endIndex;
			}
		}
		...
		template <typename Type>
		void LCU::Algorithms::Sorting<Type>::QuickSort(Type* p_pArray, size_t p_size) {
